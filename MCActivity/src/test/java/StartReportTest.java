...
public class StartReportTest {
	...
	public void testFlow() {
		...
		// ProcessDefinition processDefinition=
		// processEngine.getRepositoryService().getProcessDefinition("StartReport");
		// System.out.println(processDefinition.getName());

		int startProcCount = (int) processEngine.getRepositoryService().createProcessDefinitionQuery()
				.processDefinitionKey("StartReport").count();

		System.out.println("流程数量" + startProcCount);
		if (startProcCount == 0) {
			Deployment deployment = processEngine.getRepositoryService()// 获取流程定义和部署相关的Service
					.createDeployment()// 创建部署对象
					.addClasspathResource("diagrams/StartReport12.bpmn")
					.addClasspathResource("diagrams/StartReport12.png").deploy();// 完成部署
